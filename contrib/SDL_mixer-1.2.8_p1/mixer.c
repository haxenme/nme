...
Mix_Chunk *Mix_LoadWAV_RW(SDL_RWops *src, int freesrc)
{
	...
	chunk->length_ticks = 1000 * wavecvt.len_cvt / mixer.channels / ((mixer.format & 0xFF) / 8) / mixer.freq;
	/*
	printf("Audio len: %u samplesize: %u mixer.format masked: %u frequency : %u time: %u ms\n",
			wavecvt.len_cvt,
			samplesize,
			((mixer.format & 0xFF) /8),
			mixer.freq,
...
int Mix_PlayChannelTimed(int which, Mix_Chunk *chunk, int loops, int ticks)
{
	...
	SDL_LockAudio();
	{
		...
		if ( which >= 0 ) {
			...
			mix_channel[which].initial_loops = loops;
...
int Mix_SetChannelPosition(int which, int ms)
{
	...
	if(loops >= 0 && loopNum > loops) {
		mix_channel[which].looping = 0;
		mix_channel[which].samples = chunk->abuf + chunk->alen;
		mix_channel[which].playing = 0;
		return(1);
	}
	...
	mix_channel[which].samples = chunk->abuf + pos;
	mix_channel[which].playing = chunk->alen - pos;

	// reset the number of loops that need to be played
	if(loops > 0) {
		mix_channel[which].looping = mix_channel[which].initial_loops - loopNum;
	}
