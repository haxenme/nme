...
class Context3D 
{
   ...
   public function createTexture(width:Int, height:Int, format:Context3DTextureFormat, optimizeForRenderToTexture:Bool, streamingLevels:Int = 0):nme.display3D.textures.Texture 
   {
       var texture = new nme.display3D.textures.Texture (GL.createTexture (), width, height);     // TODO use format, optimizeForRenderToTexture and  streamingLevels?
       texturesCreated.push(texture);
        return texture;
   }
   ...
   public function setStencilReferenceValue(referenceValue:Int, readMask:Int = 0xFF, writeMask:Int = 0xFF):Void 
   {
	...
	public function setGLSLTextureAt (locationName:String, texture:TextureBase, textureIndex : Int):Void {
		...
		if (#if (haxe_ver>="4.1") Std.isOfType #else Std.is #end(texture, nme.display3D.textures.Texture)) {
			...
			GL.bindTexture(GL.TEXTURE_2D, cast(texture, nme.display3D.textures.Texture).glTexture);
